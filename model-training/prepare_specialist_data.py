...
def wav_to_mel(path,n_mels=80,n_fft=512,hop=160):
    with wave.open(str(path),"rb") as f: channels,width,rate=f.getnchannels(),f.getsampwidth(),f.getframerate(); raw=f.readframes(f.getnframes())
    ...
    spec=torch.stft(audio,n_fft=n_fft,hop_length=hop,win_length=n_fft,window=torch.hann_window(n_fft),return_complex=True).abs().pow(2); low,high=20.0,rate/2; lm,hm=1127*math.log1p(low/700),1127*math.log1p(high/700); hz=700*(torch.exp(torch.linspace(lm,hm,n_mels+2)/1127)-1); bins=torch.floor((n_fft+1)*hz/rate).long().clamp(0,n_fft//2); bank=torch.zeros(n_mels,n_fft//2+1)
...
def save(path,payload): path.parent.mkdir(parents=True,exist_ok=True); torch.save(payload,path); print(json.dumps({"output":str(path),"samples":next(iter(payload.values())).shape[0]}))
...
def build(kind,source,output):
    ...
    if kind=="embed":
    ...
    elif kind=="reranker": raise ValueError("reranker training needs numeric query/document vectors; build those from your eligible Vector dataset first")
    elif kind in {"tts","image"}:
        ids_list=[]; targets=[]
        for row in rows:
            ids_list.append(ids(row["text"] if "text" in row else row["caption"]))
            if kind=="image":
                image=Image.open(row["image"]).convert("RGB").resize((32,32)); targets.append(torch.tensor(list(image.getdata()),dtype=torch.float32).view(32,32,3).permute(2,0,1)/255)
        ...
        images=[]; labels=[]
        for row in rows:
            image=Image.open(row["image"]).convert("RGB").resize((64,64)); images.append(torch.tensor(list(image.getdata()),dtype=torch.float32).view(64,64,3).permute(2,0,1)/255); labels.append(torch.tensor(row["label"],dtype=torch.float32))
