...
def run_checked(cmd:list[str],stage:str):
 set_status("preparing",stage=stage)
 print("[bob-training] "+" ".join(map(str,cmd)),flush=True)
 subprocess.run(cmd,check=True)

def persisted_config(args,profile):
 return {"profile":profile,"english_limit":args.english_limit,"per_language_limit":args.per_language_limit,"pretrain_epochs":args.pretrain_epochs,"instruction_epochs":args.instruction_epochs,"batch_size":args.batch_size,"grad_accum":args.grad_accum,"precision":args.precision}

def train_stage(stage: str, train: Path, val: Path, tok: Path, cfg: dict, resume: bool=False, init_from: Path|None=None):
 cmd=[sys.executable,str(ROOT/"train_production.py"),"--stage",stage,"--train",str(train),"--validation",str(val),"--tokenizer",str(tok),"--output-dir",str(OUTPUT),"--profile",cfg["profile"],"--epochs",str(cfg["pretrain_epochs"] if stage=="pretrain" else cfg["instruction_epochs"]),"--batch-size",str(cfg["batch_size"]),"--grad-accum",str(cfg["grad_accum"]),"--precision",cfg["precision"],"--save-every","1"]
 ...
 state=read(STATE,{})
 write(STATE,{**state,"stage":stage,"pid":proc.pid,"updated_at":time.time()})
 while proc.poll() is None:
  c=command()
  if c in {"pause","stop"}:
   set_status("pausing" if c=="pause" else "stopping",stage=stage,pid=proc.pid,checkpoint=str(checkpoint))
   proc.terminate()
   try: proc.wait(timeout=30)
   except subprocess.TimeoutExpired:
    proc.kill(); proc.wait()
   write(CONTROL,{"command":"idle","updated_at":time.time()})
   set_status("paused" if c=="pause" else "stopped",stage=stage,checkpoint=str(checkpoint),resumable=checkpoint.exists())
   return c
  time.sleep(1)
 if proc.returncode!=0:
  set_status("failed",stage=stage,exit_code=proc.returncode,checkpoint=str(checkpoint))
...
def main():
 ...
 p.add_argument("--confirm-upstream-terms",action="store_true")
 p.add_argument("--profile",choices=["dev","125m","350m","1.3b","3b"],default=None)
 p.add_argument("--english-limit",type=int,default=100_000); p.add_argument("--per-language-limit",type=int,default=10_000)
 p.add_argument("--pretrain-epochs",type=int,default=1); p.add_argument("--instruction-epochs",type=int,default=2)
 p.add_argument("--batch-size",type=int,default=1); p.add_argument("--grad-accum",type=int,default=16)
 p.add_argument("--precision",choices=["fp32","fp16","bf16"],default="fp32")
 p.add_argument("--skip-corpus",action="store_true"); p.add_argument("--skip-deps",action="store_true"); p.add_argument("--resume",action="store_true")
 a=p.parse_args(); OUTPUT.mkdir(parents=True,exist_ok=True); data=ROOT/"data"; pre=data/"pretrain"; tok=OUTPUT/"tokenizer.json"
 write(CONTROL,{"command":"idle","updated_at":time.time()})
 if a.resume:
  saved=read(STATE,{})
  ...
  if not a.skip_corpus:
   ...
   run_checked([sys.executable,str(ROOT/"build_pretraining_corpus.py"),"--confirm-upstream-terms","--english-limit",str(cfg["english_limit"]),"--per-language-limit",str(cfg["per_language_limit"])],"knowledge_corpus")
   run_checked([sys.executable,str(ROOT/"prepare_pretraining.py")],"pretraining_split")
  ...
  run_checked([sys.executable,str(ROOT/"build_final_dataset.py")],"instruction_corpus")
  run_checked([sys.executable,str(ROOT/"prepare_dataset.py"),"--input",str(data/"source.jsonl")],"instruction_split")
  ...
  write(STATE,{"stage":"pretrain","config":cfg,"updated_at":time.time()})
  result=train_stage("pretrain",pre/"train.jsonl",pre/"validation.jsonl",tok,cfg)
  if result in {"pause","stop"}: return
  ...
  write(STATE,{"stage":"instruction","config":cfg,"updated_at":time.time()})
  result=train_stage("instruction",data/"train.jsonl",data/"validation.jsonl",tok,cfg,init_from=init_checkpoint)
  if result in {"pause","stop"}: return
 write(STATE,{"stage":"done","config":cfg,"updated_at":time.time()})
 final_checkpoint=OUTPUT/"best.pt" if (OUTPUT/"best.pt").exists() else OUTPUT/"latest.pt"
 set_status("completed",stage="done",profile=cfg.get("profile"),progress=1.0,checkpoint=str(final_checkpoint))
