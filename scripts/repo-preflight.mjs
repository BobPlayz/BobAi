...
async function main() {
  ...
  const handoffParagraphs = handoff.split(/\n\s*\n/).map((part) => part.trim()).filter(Boolean);
  if (handoffParagraphs.length < 3 || handoffParagraphs.some((part) => part.startsWith("#"))) throw new Error("BobAI handoff must remain paragraph-only");
