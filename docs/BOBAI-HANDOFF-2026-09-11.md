BobAI remains a single user-facing Bob model with a central tool policy boundary. Repository work now covers bounded tool execution, approvals, validation, cancellation, network-target checks, provider limits, bundle integrity, audio limits, webhook limits, result limits, agent-plan bounds, complete checked-in API test execution through the root test command, production validation of the OAuth redirect base, and bounded malware-scanner responses before parsing. Obsolete lockfile workflows with repository write access were removed.
