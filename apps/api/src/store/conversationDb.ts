...
export async function dbListConversations(userId: string, workspaceId: string, includeArchived = false) {
  const database = await getDb(); if (!database) return null;
  const { asc, desc } = await import("drizzle-orm"); const { conversations, messages } = database;
  ...
  const rows = await database.db.select().from(conversations).where(where).orderBy(desc(conversations.isPinned), desc(conversations.updatedAt));
  return Promise.all(rows.map(async (conversation) => ({ ...conversation, messages: await database.db.select({ id: messages.id, role: messages.role, content: messages.content, model: messages.model, status: messages.status, attachments: messages.attachments, createdAt: messages.createdAt }).from(messages).where(eq(messages.conversationId, conversation.id)).orderBy(asc(messages.createdAt)) })));
}

export async function dbGetConversation(id: string, userId: string, workspaceId: string) {
  const database = await getDb(); if (!database) return null; const { asc } = await import("drizzle-orm"); const { conversations, messages } = database;
...
export async function dbSaveConversation(input: { id: string; userId: string; workspaceId: string; title: string; messages: Array<{ id?: string; role: string; content: string; model?: string | null; status?: string; attachments?: unknown }> }) {
  ...
  await database.db.transaction(async (tx) => {
    ...
    for (const message of input.messages) if (message.id) await tx.insert(messages).values({ id: message.id, conversationId: input.id, role: message.role, content: message.content, model: message.model ?? null, status: message.status ?? "completed", attachments: message.attachments ?? null });
  }); return true;
}
