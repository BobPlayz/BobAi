...
export async function dbListConversations(userId: string, workspaceId: string) {
  ...
  const { and, asc, desc } = await import("drizzle-orm");
...
export async function dbDeleteConversation(id: string, userId: string, workspaceId: string) {
  ...
  const result = await database.db
    .delete(database.conversations)
    .where(
      and(
        eq(database.conversations.id, id),
        eq(database.conversations.userId, userId),
        eq(database.conversations.workspaceId, workspaceId),
      ),
    )
