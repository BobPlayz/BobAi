...
async function hashPassword(password: string) {
  ...
  const derived = (await scrypt(password, salt, 64, SCRYPT)) as Buffer;
...
async function verifyPassword(password: string, stored: string | null) {
  ...
  const actual = (await scrypt(password, Buffer.from(saltText, "base64url"), expected.length, SCRYPT)) as Buffer;
...
export async function authenticateAccessToken(accessToken: string) {
  const [result] = await db.select({ session: sessions, user: users }).from(sessions).innerJoin(users, eq(users.id, sessions.userId)).where(and(
    eq(sessions.accessTokenHash, hashToken(accessToken)),
...
export async function refresh(refreshToken: string) {
  ...
  const now = new Date();
  const [session] = await db.update(sessions).set({ revokedAt: now, isActive: false, updatedAt: now }).where(and(
