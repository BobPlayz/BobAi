...
const isSpecialistCapability = (capability: RuntimeCapability): capability is Exclude<RuntimeCapability, "chat" | "coding"> => capability !== "chat" && capability !== "coding";
export class ModelRuntimeManager {
 ...
 if(this.loadedMemory()+entry.model.memoryMb>MODEL_BUDGET_MB)throw new Error("model memory budget is exhausted");entry.state="loading";entry.error=undefined;try{if(id==="bob"||id==="coder")await this.loadNative(id);else{const capability=entry.model.capability;if(!isSpecialistCapability(capability))throw new Error(`invalid specialist capability for ${id}`);if(!specialistConfigured(capability))throw new Error(`no ${entry.model.id} artifact configured`);await ensureSpecialist(capability);entry.stop=async()=>{await stopSpecialist(capability);};}entry.state="ready";entry.loadedAt=new Date().toISOString();entry.lastUsedAt=entry.loadedAt;}catch(error){entry.state="error";entry.error=error instanceof Error?error.message:"model load failed";throw error;}}
