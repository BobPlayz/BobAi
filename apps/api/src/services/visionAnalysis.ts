...
function parseImage(value: string) {
  ...
  if (!match) throw new Error("vision accepts only PNG, JPEG, or WebP data URLs");
...
function readPng(buffer: Buffer): VisionImageReport | null {
...
export async function analyzeVisionImages(images: string[], prompt = "Analyze these visual inputs for a coding task. Identify layout, components, spacing, typography, colors, states, visual defects, and implementation-relevant details. Do not invent details that are not visible.") {
  if (!Array.isArray(images) || images.length < 1 || images.length > MAX_IMAGES) throw new Error(`vision accepts 1-${MAX_IMAGES} images`);
  if (typeof prompt !== "string" || prompt.length > MAX_PROMPT) throw new Error("vision prompt is too long");
  ...
  return { reports, model: process.env.BOBAI_VISION_MODEL, analysis };
}
