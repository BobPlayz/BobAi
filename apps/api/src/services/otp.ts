...
export const requestEmailOtp = async (email: string) => {
  ...
  const [recent] = await db.select({ id: emailOtps.id }).from(emailOtps).where(
    and(eq(emailOtps.userId, user.id), gt(emailOtps.createdAt, new Date(Date.now() - 60_000)))
  ).limit(1);
  if (recent) return;
...
export const verifyEmailOtp = async (email: string, code: string) => {
  ...
  const [otp] = await db.select().from(emailOtps).where(
    and(eq(emailOtps.userId, user.id), isNull(emailOtps.consumedAt), gt(emailOtps.expiresAt, new Date()))
