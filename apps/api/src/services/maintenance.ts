...
export function startMaintenance() { void runMaintenance(); void startAutomationWorker().catch((error) => { if (process.env.NODE_ENV !== "production") console.warn("automation worker startup failed", error); }); startReminderWorker(); startResearchBriefWorker(); const timer = setInterval(() => { void runMaintenance(); }, INTERVAL_MS); timer.unref(); return timer; }
