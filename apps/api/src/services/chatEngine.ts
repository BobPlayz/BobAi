...
export function normalizeMessages(input: unknown): ChatMessage[] {
  ...
  return input.map((message: unknown) => {
    ...
    return { role, content: typeof value?.content === "string" ? value.content : String(value?.content ?? "") };
  });
}
...
export function prepareChat(input: ChatInput) {
  ...
  const system = buildSystemPrompt(personality, memoryContext, language, responseStyle);
...
export async function runChat(messages: ProviderMessage[], modelId?: string) {
  ...
  if (result.content.length > MAX_RESPONSE_CHARACTERS) throw new Error("model response exceeded the safety limit");
  return { message: { content: result.content }, model: result.model, provider: result.provider };
}

export async function runStream(messages: ProviderMessage[], onToken: (token: string) => void, modelId?: string): Promise<string> {
