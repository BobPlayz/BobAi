...
const DEFAULT_MODEL = "bob-0.1-native";
...
export class BobModelProvider {
  async chat(messages: ProviderMessage[], provider: "bob" | "coding" = "bob", modelOverride?: string): Promise<ProviderResult> { const model = selectedModel(provider, modelOverride); const content = await bobNativeModel.generate(messages, { maxTokens: 512 }); if (!content || content.length > MAX_RESPONSE_CHARS) throw new Error("BobAI native model returned an invalid response"); return { content, model, provider }; }
  async stream(messages: ProviderMessage[], onToken: (token: string) => void, provider: "bob" | "coding" = "bob", modelOverride?: string): Promise<ProviderResult> { const model = selectedModel(provider, modelOverride); const content = await bobNativeModel.generate(messages, { maxTokens: 512, onToken }); if (!content || content.length > MAX_RESPONSE_CHARS) throw new Error("BobAI native model returned an invalid response"); return { content, model, provider }; }
