...
type QueueJob = { id: string; description: string; kind?: AgentTaskKind; skills?: AgentSkillId[]; mode?: string; context?: { workspaceId?: string; createdBy?: string }; attempts: number; status: "queued" | "running" | "completed" | "failed"; createdAt: string; result?: unknown; error?: string };
const queue: QueueJob[] = [];
const jobs = new Map<string, QueueJob>();
let running = 0;
const concurrency = Math.max(1, Number(process.env.BOBAI_AGENT_CONCURRENCY || 2));
...
export function enqueueAgentTask(input: Omit<QueueJob, "id" | "attempts" | "status" | "createdAt">) {
  ...
  const job: QueueJob = { ...input, id: randomUUID(), attempts: 0, status: "queued", createdAt: new Date().toISOString() };
  queue.push(job); jobs.set(job.id, job); void drain(); return job;
}
...
async function drain() {
  while (running < concurrency) {
    ...
    if (!job) return;
    job.status = "running"; job.attempts += 1; running += 1;
    void run(job).finally(() => { running -= 1; void drain(); });
  }
}
...
async function run(job: QueueJob) {
  try {
    ...
    if (job.attempts < maxAttempts) { job.status = "queued"; queue.push(job); } else job.status = "failed";
  }
}
