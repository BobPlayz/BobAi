...
const concurrency = Math.max(1, Math.min(4, Number(process.env.BOBAI_AGENT_CONCURRENCY || 1))); const maxAttempts = Math.max(1, Math.min(5, Number(process.env.BOBAI_AGENT_MAX_ATTEMPTS || 2)));
export function enqueueAgentTask(input: Omit<QueueJob, "id" | "attempts" | "status" | "createdAt">) { if (queue.length >= 100) throw new Error("agent queue is full"); const job: QueueJob = { ...input, id: randomUUID(), attempts: 0, status: "queued", createdAt: new Date().toISOString(), context: { ...input.context, taskId: undefined } }; job.context = { ...job.context, taskId: job.id }; queue.push(job); jobs.set(job.id, job); void persistAgentTask({ id: job.id, workspaceId: job.context?.workspaceId, createdBy: job.context?.createdBy, title: job.description.slice(0, 120), description: job.description, type: job.kind || "coding", status: "queued", payload: { mode: job.mode, skills: job.skills, budget: job.context?.budget, checkpointToken: job.context?.checkpointToken } }).catch(() => false); void drain(); return job; }
