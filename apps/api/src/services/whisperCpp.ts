...
export async function transcribeLocal(input: Buffer, options?: Record<string, unknown> | null) {
  ...
  try {
    ...
    const language = typeof safeOptions.language === "string" && /^[A-Za-z-]{2,12}$/.test(safeOptions.language) ? safeOptions.language : "";
