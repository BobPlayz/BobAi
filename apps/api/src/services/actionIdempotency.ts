...
export async function cleanupIdempotency(now = new Date()) { const staleProcessingAt = new Date(now.getTime() - PROCESSING_TTL_MS); const deleted = await db.delete(actionIdempotency).where(or(sql`${actionIdempotency.expiresAt} <= ${now.toISOString()}`, and(eq(actionIdempotency.status, "processing"), sql`${actionIdempotency.updatedAt} <= ${staleProcessingAt.toISOString()}`))).returning({ id: actionIdempotency.id }); return deleted.length; }
