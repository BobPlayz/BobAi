...
export async function requestPasswordReset(email: string) {
  ...
  await db
    ...
    .set({ isActive: false })
    .where(and(eq(passwordResets.userId, user.id), eq(passwordResets.isActive, true)));
...
export async function resetPassword(rawToken: string, password: string) {
  ...
  await db
    .update(users)
    .set({ passwordHash: stored, updatedAt: now })
    .where(eq(users.id, reset.userId));

  await db
    .update(sessions)
    .set({ revokedAt: now, isActive: false, updatedAt: now })
    .where(
      and(
        eq(sessions.userId, reset.userId),
