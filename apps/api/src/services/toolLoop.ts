...
function externalProviderUrl(toolId: string): string | undefined {
  switch (toolId) {
    case "browser":
    case "website-test": return process.env.BOBAI_BROWSER_PROVIDER_URL?.trim();
...
async function callConfiguredProvider(tool: BobTool, args: Record<string, unknown>): Promise<unknown> {
  ...
  let url: URL;
  try { url = new URL(raw); } catch { throw new Error(`${tool.id} provider URL is invalid`); }
  if (!["https:", "http:"].includes(url.protocol)) throw new Error(`${tool.id} provider URL is invalid`);
  if (process.env.NODE_ENV === "production" && url.protocol !== "https:") throw new Error(`${tool.id} provider must use HTTPS in production`);
  ...
  try {
    ...
    if (text.length > tool.maxResultBytes) throw new Error("tool result exceeds the configured limit");
...
const EXECUTORS: Record<string, Executor> = {
  ...
  browser: async (args) => callConfiguredProvider(getTool("browser")!, args),
  "website-test": async (args) => callConfiguredProvider(getTool("website-test")!, args),
  ...
  automation: async (args, context) => {
    ...
    return queueBackgroundTask({ description: `run approved automation ${automationId}`, mode: context.mode, context: { workspaceId: context.workspaceId, createdBy: context.userId, automationId } });
  },
