...
import { validatePcm16Wav } from "./wavSafety.js";
...
export async function convertMedia(input: Buffer, format: string, options?: Record<string, unknown> | null) { if (input.length > 32 * 1024 * 1024) throw new Error("media exceeds 32 MB limit"); const normalized = format.toLowerCase(); if (!ALLOWED_NATIVE_FORMATS.has(normalized)) throw new Error("native BobMedia currently supports PCM16 WAV only; no external FFmpeg process is used"); validatePcm16Wav(input, 32 * 1024 * 1024); nativeMedia.require().probeWav(input); const safeOptions = asOptions(options); if (safeOptions.audioOnly !== undefined && safeOptions.audioOnly !== true) throw new Error("invalid audioOnly option"); return { media: input.toString("base64"), format: normalized, mimeType: "audio/wav", engine: "BobMedia/native" }; }
export async function mediaStatus() { return nativeMedia.status(); }
export async function probeMedia(input: Buffer) { if (input.length > 32 * 1024 * 1024) throw new Error("media exceeds 32 MB limit"); validatePcm16Wav(input, 32 * 1024 * 1024); return nativeMedia.require().probeWav(input); }
