...
import { assertPublicTargetUrl, boundedText } from "./httpSafety.js";
const MAX_PAYLOAD_BYTES = 512 * 1024, MAX_ATTEMPTS = 4, SIGNATURE_TTL_SECONDS = 300, REQUEST_TIMEOUT_MS = 15_000;
function hmac(secret: string, value: string) { return createHmac("sha256", secret).update(value).digest("hex"); }
function validUrl(value: string) { try { const url = new URL(value); return url.protocol === "https:" && !url.username && !url.password && !url.hash; } catch { return false; } }
...
export async function deliverWebhook(webhookId: string, workspaceId: string, event: string, payload: unknown) {
  ...
  await assertPublicTargetUrl(hook.url);
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    const timestamp = Math.floor(Date.now() / 1000); const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);
    try {
      ...
      lastError = `webhook returned ${response.status}`; if (![408, 409, 425, 429].includes(response.status) && response.status < 500) break;
    } catch (error) { lastError = error instanceof Error ? error.message : "webhook delivery failed"; } finally { clearTimeout(timer); }
...
export function verifyWebhookSignature(secret: string, timestamp: string, eventId: string, payload: string, supplied: string) { if (!secret || !/^\d{10}$/.test(timestamp) || !eventId || eventId.length > 200) return false; const ts = Number(timestamp); if (!Number.isSafeInteger(ts) || Math.abs(Math.floor(Date.now() / 1000) - ts) > SIGNATURE_TTL_SECONDS) return false; return safeCompare(signature(secret, ts, eventId, payload), supplied); }
