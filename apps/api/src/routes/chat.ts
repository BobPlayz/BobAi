...
router.post("/", async (req, res) => {
  try {
    ...
    const prepared = prepareChat({ messages: req.body?.messages, personality: req.body?.personality, modelId: req.body?.modelId, memoryContext: relevantMemories(memories, typeof latestText === "string" ? latestText : "") });
    ...
    const assistantMessage = { id: randomUUID(), role: "assistant", content: response.message.content, model: prepared.modelId || process.env.OLLAMA_DEFAULT_MODEL || "qwen2.5:3b", status: "completed" };
    ...
    return res.json({ reply: response.message.content, title: prepared.title, agent: "bob", model: assistantMessage.model, streamReady: true, conversationId, persisted });
  } catch (error) {
