...
function relevantMemories(memories: Array<{ key: string; value: string }> | null, query: string) {
  ...
  return memories
    ...
    .slice(0, 12)
    .map((item) => `${item.memory.key}: ${item.memory.value}`);
}
...
router.post("/", async (req, res) => {
  try {
    ...
    const latestText = Array.isArray(req.body?.messages)
      ? [...req.body.messages].reverse().find((message: any) => message?.role === "user")?.content || ""
      : "";
