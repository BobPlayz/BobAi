...
router.post("/", async (req, res) => {
  try {
    ...
    if (requestedProjectId) {
      ...
      const [project] = await db.select({ id: projects.id, settings: projects.settings, workspaceId: projects.workspaceId, ownerId: projects.ownerId }).from(projects).where(and(eq(projects.id, requestedProjectId), eq(projects.workspaceId, workspace.id), eq(projects.ownerId, req.user!.id), eq(projects.archived, false))).limit(1);
      if (!project) return res.status(404).json({ error: "project not found" });
      projectId = project.id;
      const value = project.settings;
      if (value && typeof value === "object" && !Array.isArray(value)) { const instructions = (value as Record<string, unknown>).instructions; if (typeof instructions === "string") projectInstructions = instructions.slice(0, MAX_PROJECT_INSTRUCTIONS); }
    }
    ...
    const prepared = prepareChat({ messages: req.body?.messages, personality, modelId: typeof req.body?.modelId === "string" ? req.body.modelId : storedModel, language: storedLanguage, responseStyle: storedResponseStyle, memoryContext: relevantMemories(memories) });
    ...
    if (prepared.memoryRequest && prepared.latestUserMessage) {
      ...
      const stored = await dbRemember({ workspaceId: workspace.id, userId: req.user!.id, projectId, key: "explicit memory", value: prepared.latestUserMessage.content.trim() });
      if (!stored) return res.status(503).json({ error: "memory storage unavailable" });
      return res.json({ reply: "got it. i will remember that for future conversations.", title: prepared.title, memoryStored: true, agent: "bob", projectId: projectId ?? null });
    }
    ...
    if (prepared.latestUserMessage && isCodingTask(prepared.latestUserMessage.content)) {
      const job = queueBackgroundTask({ description: prepared.latestUserMessage.content, mode: typeof req.body?.mode === "string" ? req.body.mode : undefined, context: { workspaceId: workspace.id, createdBy: req.user?.id } });
      ...
      return res.status(202).json({ reply: `i've queued that for the coding agents. job ${job.id} is running only when a worker is available.`, title: prepared.title, agent: "bob", backgroundJobId: job.id, background: true, conversationId, projectId: projectId ?? null });
    }
