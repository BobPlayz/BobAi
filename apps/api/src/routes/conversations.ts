...
async function requireContext(req: AuthenticatedRequest, res: Response) {
  ...
  if (!/^[0-9a-f-]{36}$/i.test(requested)) { res.status(400).json({ error: "invalid workspace context" }); return null; }
...
router.get("/:id", async (req, res) => { try { const context = await requireContext(req as AuthenticatedRequest, res); if (!context) return; const conversation = await dbGetConversation(req.params.id as string, context.userId, context.workspaceId); if (!conversation) return res.status(404).json({ error: "conversation not found" }); return res.json(conversation); } catch { return res.status(500).json({ error: "failed to get conversation" }); } });
router.post("/", async (req, res) => { const conversation = req.body as Record<string, unknown>; if (typeof conversation?.id !== "string" || typeof conversation?.title !== "string" || !Array.isArray(conversation?.messages)) return res.status(400).json({ error: "id, title, and messages are required" }); try { const context = await requireContext(req as AuthenticatedRequest, res); if (!context) return; const saved = await dbSaveConversation({ id: conversation.id, userId: context.userId, workspaceId: context.workspaceId, title: conversation.title.slice(0, 200), messages: conversation.messages as Array<{ id?: string; role: string; content: string; model?: string | null; status?: string; attachments?: unknown }> }); if (!saved) return res.status(503).json({ error: "persistent storage unavailable" }); return res.json({ success: true, persistent: true, workspaceId: context.workspaceId }); } catch { return res.status(500).json({ error: "failed to save conversation" }); } });
router.delete("/:id", async (req, res) => { try { const context = await requireContext(req as AuthenticatedRequest, res); if (!context) return; const deleted = await dbDeleteConversation(req.params.id as string, context.userId, context.workspaceId); if (!deleted) return res.status(404).json({ error: "conversation not found" }); return res.json({ success: true, persistent: true }); } catch { return res.status(500).json({ error: "failed to delete conversation" }); } });
