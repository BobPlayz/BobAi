...
import { decideTools } from "../services/toolDecision.js";
...
function shouldRouteTools(text: string) { return /\b(search|research|verify|latest|current|browse|website|open (?:the )?site|look up|generate (?:an )?(?:image|video)|create (?:an )?(?:image|video|diagram)|edit (?:an )?image|use (?:the )?computer|paint|blender|analyze (?:this )?data|run (?:the )?automation|transcribe|synthesize|voice|music|sketch to ui)\b/i.test(text); }
router.post("/", async (req, res) => {
  const { send } = initSSE(res); const conversationId = typeof req.body?.conversationId === "string" && req.body.conversationId.trim() ? req.body.conversationId.trim() : randomUUID(); let assistantId: string | null = null; let workspaceId: string | null = null;
  try {
    const workspace = await ensurePersonalWorkspace(req.user!.id); workspaceId = workspace.id;
    ...
    const enrichedMessages = await enrichMessagesWithVision(req.body?.messages, latestUserText(req.body?.messages)); const latestText = latestUserText(enrichedMessages);
    const memories = memoryEnabled && latestText ? await dbRecallRelevant(workspace.id, req.user!.id, latestText, 12) : [];
    const fileKnowledge = latestText ? await searchDocumentChunks(workspace.id, req.user!.id, latestText, undefined, 8) : [];
    const knowledgeContext = fileKnowledge.map((item) => `[file ${String(item.uploadId || "unknown")}, chunk ${String(item.chunkIndex ?? "?")}] ${typeof item.content === "string" ? item.content.slice(0, 5_000) : ""}`).filter((item) => item.length > 20);
    const prepared = await prepareChat({ messages: enrichedMessages, personality: typeof req.body?.personality === "string" ? req.body.personality : settingValue(rows, "personality"), modelId: typeof req.body?.modelId === "string" ? req.body.modelId : settingValue(rows, "model"), language: settingValue(rows, "language"), responseStyle: settingValue(rows, "responseStyle"), responseLength: settingValue(rows, "responseLength"), tone: settingValue(rows, "tone"), customInstructions: settingValue(rows, "customInstructions"), memoryContext: memoryContext(memories), knowledgeContext });
    ...
    assistantId = randomUUID(); const initialMessages = [...persistedInputMessages(prepared.messages), { id: assistantId, role: "assistant", content: "", model: null, status: "pending" }]; const persisted = await dbSaveConversation({ id: conversationId, userId: req.user!.id, workspaceId: workspace.id, title: prepared.title, messages: initialMessages }); if (!persisted) throw new Error("persistent chat storage unavailable"); await dbUpdateMessageStatus({ conversationId, messageId: assistantId, userId: req.user!.id, workspaceId: workspace.id, status: "streaming" });
    let disconnected = false; const onClose = () => { disconnected = true; }; req.once("close", onClose);
    try { const result = await runStream(prepared.providerMessages, (token) => { if (!disconnected && !res.writableEnded) send("token", { token }); }, prepared.modelId); const finalStatus = disconnected ? "cancelled" : "completed"; await dbUpdateMessageStatus({ conversationId, messageId: assistantId!, userId: req.user!.id, workspaceId: workspace.id, status: finalStatus, content: result.content, model: result.model }); if (disconnected || res.writableEnded) return; send("done", { reply: result.content, title: prepared.title, model: result.model, provider: result.provider, conversationId, persisted: true, status: finalStatus }); return res.end(); } catch (error) { await dbUpdateMessageStatus({ conversationId, messageId: assistantId!, userId: req.user!.id, workspaceId: workspace.id, status: disconnected ? "cancelled" : "failed", content: "" }).catch(() => false); throw error; } finally { req.off("close", onClose); }
  } catch (error) { if (process.env.NODE_ENV !== "production") console.error("STREAM ROUTE ERROR:", error); if (assistantId && workspaceId) await dbUpdateMessageStatus({ conversationId, messageId: assistantId, userId: req.user!.id, workspaceId, status: "failed" }).catch(() => false); if (!res.writableEnded) { send("error", { message: process.env.NODE_ENV === "production" ? "stream failed" : error instanceof Error ? error.message : "stream failed" }); res.end(); } }
});
