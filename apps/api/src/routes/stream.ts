...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const persisted = await dbSaveConversation({ id: conversationId, userId: req.user!.id, workspaceId: workspace.id, title: prepared.title, messages: [...prepared.messages, { id: randomUUID(), role: "assistant", content: result.content, model: result.model, status: "completed" }] }).catch(() => false);
      if (disconnected || res.writableEnded) return;
      send("done", { reply: result.content, title: prepared.title, model: result.model, provider: result.provider, conversationId, persisted });
      return res.end();
