...
async function checkOllama() {
  const base = process.env.OLLAMA_HOST || process.env.OLLAMA_BASE_URL || process.env.OLLAMA_URL;
  if (!base) return false;
  try {
    const response = await fetch(`${base.replace(/\/$/, "")}/api/tags`, { signal: AbortSignal.timeout(3000) });
    return response.ok;
  } catch {
    return false;
  }
...
healthRouter.get("/ready", async (_req, res) => {
  ...
  const ready = ollama && (!checks.databaseConfigured || database);
