...
router.post("/register", async (req, res) => {
  ...
  try {
    ...
    return res.status(201).json({ expiresIn: session.expiresIn, user });
  } catch (error) {
...
router.post("/refresh", async (req, res) => {
  ...
  if (!session) { res.clearCookie(ACCESS_COOKIE, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/" }); res.clearCookie(REFRESH_COOKIE, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/" }); return res.status(401).json({ error: "invalid or expired session" }); }
  setSessionCookies(res, session);
  return res.json({ expiresIn: session.expiresIn });
});
...
router.post("/logout", async (req, res) => {
  ...
  res.clearCookie(ACCESS_COOKIE, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/" });
  res.clearCookie(REFRESH_COOKIE, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/" });
