...
async function main() {
  ...
  const { verifyTotp } = await import("../src/services/mfa.js");
  ...
  test("unsafe request structures are rejected", async () => {
    const response = await fetch(`${base}/v1/auth/login`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ constructor: { polluted: true } }) });
