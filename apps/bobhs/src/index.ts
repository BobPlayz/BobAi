...
async function controllerHandler(req: IncomingMessage, res: ServerResponse) {
  ...
  if (req.method === "POST" && url.pathname === "/v1/nodes/register") { if (!authJoin(req)) return json(res, 401, { error: "unauthorized" }); const input = await body(req); if (!validName(input.name)) return json(res, 400, { error: "invalid node name" }); if (state.nodes.some(n => n.name === input.name && n.status !== "offline")) return json(res, 409, { error: "node name already exists" }); const token = randomBytes(32).toString("base64url"); const node: NodeRecord = { id: randomUUID(), name: input.name, tokenHash: hashToken(token), status: "online", lastSeen: new Date().toISOString(), cpuCount: Math.min(256, Math.max(1, Number(input.cpuCount) || 1)), memoryMb: Math.min(1048576, Math.max(256, Number(input.memoryMb) || 256)), freeMemoryMb: Math.min(1048576, Math.max(0, Number(input.freeMemoryMb) || Number(input.memoryMb) || 256)), load1: 0, activeJobs: 0, labels: stringList(input.labels, 20), capabilities: stringList(input.capabilities, 32), models: stringList(input.models, 64) }; state.nodes.push(node); await saveState(state); return json(res, 201, { node: { ...node, tokenHash: undefined }, token }); }
