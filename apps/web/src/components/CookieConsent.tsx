...
export default function CookieConsent() {
  ...
  if (consent === "accepted") {
    ...
    return domain ? <Script defer data-domain={window.location.hostname} src={`${domain.replace(/\/$/, "")}/js/script.js`} /> : null;
  }
  ...
  return (
    <aside className="fixed inset-x-4 bottom-4 z-[60] mx-auto max-w-xl rounded-2xl border border-white/10 bg-slate-950/90 p-4 shadow-2xl backdrop-blur-xl" aria-label="Analytics consent">
      ...
      <div className="mt-3 flex gap-2">
        <button onClick={() => { localStorage.setItem(STORAGE_KEY, "accepted"); setConsent("accepted"); }} className="rounded-xl bg-cyan-300 px-4 py-2 text-sm font-semibold text-slate-950">Allow analytics</button>
        <button onClick={() => { localStorage.setItem(STORAGE_KEY, "declined"); setConsent("declined"); }} className="rounded-xl border border-white/10 px-4 py-2 text-sm text-white/70 hover:text-white">No thanks</button>
      </div>
    </aside>
  );
}
