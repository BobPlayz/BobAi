...
async function request(path: string, init: RequestInit = {}, retry = true) {
  ...
  if (["POST", "PUT", "PATCH", "DELETE"].includes((init.method || "GET").toUpperCase())) headers.set("X-CSRF-Protection", "1");
  ...
  if (res.status === 401 && retry) {
    ...
    if (refreshed?.accessToken) {
      ...
      if (["POST", "PUT", "PATCH", "DELETE"].includes((init.method || "GET").toUpperCase())) retryHeaders.set("X-CSRF-Protection", "1");
