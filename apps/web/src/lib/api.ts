...
export async function sendMessage(messages: ChatMessage[], personality = "", modelId?: string) { const cleanMessages = messages.map((m) => ({ role: m.role, content: m.content })); const res = await request("/chat", { method: "POST", body: JSON.stringify({ messages: cleanMessages, personality, ...(modelId ? { modelId } : {}) }) }); if (!res.ok) throw new Error((await res.text()) || "Failed to send message"); return res.json(); }
