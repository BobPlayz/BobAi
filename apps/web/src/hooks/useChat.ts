...
export function useChat() {
  ...
  async function send() {
    ...
    if (!text || !activeConversation || loadingConversationId) return;
    const conversationId = activeId;
    setInput(""); setLoadingConversationId(conversationId);
    ...
    try {
      ...
      if (result.imagePrompt) {
        const imageResult = await generateImage(result.imagePrompt) as { images: GeneratedImage[] };
        aiMessage = { id: crypto.randomUUID(), role: "assistant", content: "", images: imageResult.images.filter((img): img is GeneratedImage & { url: string } => typeof img.url === "string").map((img): ChatImage => ({ id: crypto.randomUUID(), url: img.url, prompt: img.prompt })) };
      } else aiMessage = { id: crypto.randomUUID(), role: "assistant", content: result.reply };
  ...
  async function regenerateLastAssistant() {
    ...
    const withoutAssistant = messages.filter((m) => m.id !== lastAssistant.id);
    setLoadingConversationId(activeId);
  ...
  return { conversations, visibleConversations, activeConversation, activeId, setActiveId, input, setInput, search, setSearch, loadingConversationId, settings, setSettings, uploadingFiles, uploadProgress, send, handleFiles, regenerateLastAssistant, newChat, togglePin };
}
