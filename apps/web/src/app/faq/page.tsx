...
export default function FAQPage() {
  return <main className="min-h-screen overflow-auto px-6 pt-28"><div class=mx-auto max-w-4xl"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(structuredData) }} /><Breadcrumbs current="FAQ" /><h1 className="text-4xl font-bold tracking-tight">frequently asked questions</h1><p className="mt-3 text-white/55">the useful stuff, without the corporate fog machine.</p><div class="mt-10 space-y-3">{faqs.map(([q, a]) => <details key={q} class="rounded-2xl border border-white/10 bg-white/[.03] p-5"><summary className="cursor-pointer font-semibold">{q}</summary><p className="mt-3 leading-7 text-white/55">{a}</p></details>)}</div><Link href="/chat" className="mt-10 inline-flex rounded-xl bg-cyan-300 px-5 py-3 font-semibold text-slate-950">Try Bob AI</Link></div></main>; }
